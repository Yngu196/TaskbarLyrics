...
// WebSocket 协议选择说明（安全审查已确认，非问题）：
//
// 当前使用 ws:// 而非 wss://，原因如下：
//   1. MoeKoeMusic 内置 apiService 仅提供 ws:// 端点（监听 127.0.0.1:6520），
//      不提供 wss://（未实现 TLS 证书管理/配置）。
//   2. ws:// 绑定在 127.0.0.1 环回地址，数据不会离开本机网络栈，
//      外部网络攻击者无法嗅探或篡改 localhost 流量。
//   3. 同机恶意进程嗅探 localhost 是操作系统级安全问题（需管理员权限），
//      非本插件可控范围。
// 若 MoeKoeMusic 后续支持 wss://，将 DEFAULT_WS_PROTOCOL 改为 'wss' 即可升级，
// 高级用户也可通过 chrome.storage.local.wsProtocol 手动覆盖。
...
// ---- Token 管理（加密存储） ----
// 
// 安全策略：authToken 不直接明文存入 chrome.storage.local。
// 加密密钥存储在 chrome.storage.session（内存级，不清盘），
// 加密后的令牌密文存储在 chrome.storage.local（落盘）。
// 两者分离：仅持有密钥才能解密密文；密钥随浏览器进程退出而销毁。
// 浏览器重启后密钥丢失 → 旧密文无法解密 → 自动生成新令牌（无缝轮换）。
let cachedAuthToken = null;
let saveTimeout = null;           // scheduleStateSave 节流定时器

// Web Crypto 加密/解密辅助函数（AES-GCM, cryptoKey 存储在 chrome.storage.session）
const TOKEN_STORAGE_KEY = 'authTokenEncrypted';
const TOKEN_KEY_SESSION_KEY = 'authTokenCryptoKey';

async function getCryptoKey() {
    const stored = await chrome.storage.session.get(TOKEN_KEY_SESSION_KEY);
    if (stored[TOKEN_KEY_SESSION_KEY]) {
        // 从 session storage 恢复 JWK → CryptoKey
        return crypto.subtle.importKey(
            'jwk', stored[TOKEN_KEY_SESSION_KEY],
    ...
    await chrome.storage.session.set({ [TOKEN_KEY_SESSION_KEY]: jwk });
    return key;
}

async function encryptToken(plainToken) {
    ...
    // 返回 IV + 密文 的 Base64 拼接（IV 不需保密，但每次加密必须唯一）
...
async function decryptToken(encryptedToken) {
    ...
    if (parts.length !== 2) throw new Error('令牌密文格式错误');
...
// 获取本地鉴权令牌（首次自动生成并持久化，存储时加密）
async function getAuthToken() {
    if (cachedAuthToken) return cachedAuthToken;
    try {
        const result = await chrome.storage.local.get(TOKEN_STORAGE_KEY);
        if (result[TOKEN_STORAGE_KEY]) {
            cachedAuthToken = await decryptToken(result[TOKEN_STORAGE_KEY]);
            return cachedAuthToken;
        }
    } catch (_) {
        // 解密失败（如浏览器重启后密钥丢失）→ 清除旧密文，走生成新令牌逻辑
    ...
    await chrome.storage.local.set({ [TOKEN_STORAGE_KEY]: encrypted });
...
//
// 安全审查说明（防误判）：
//   本函数完整且未被截断。重连计数使用标量 reconnectAttempts（非数组），
//   不存在数组越界风险。重连上限由 MAX_RECONNECT_ATTEMPTS 常量控制，
//   到达上限后设置 reconnectAborted 标记并停止自动重连。
//   手动重连通过 resetReconnectState() 清零计数器 + 清除中止标记。
//   连接成功时在 ws.onopen 中同样清零计数器（L~218）。
...
restoreState().then(() => {
    ...
    if (!reconnectAborted) {
        connectWebSocket();
        console.log(`[TaskbarLyrics] 插件已加载 (v1.0.1 Native Host 模式, WS端口: ${wsPort})`);
    } else {
